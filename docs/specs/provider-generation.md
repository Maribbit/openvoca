...
多数模型默认开启深度思考，这在词汇学习这类任务上只增加延迟与成本。关闭它的字段各家不同，
且**没有共同形状**：

| 提供商 | 字段 | 关闭方式 |
|---|---|---|
| OpenAI | `reasoning_effort` | `"none"` |
| DeepSeek | `reasoning_effort`（兼容值 `minimal`/`medium`/`xhigh` 会被映射） | `"none"` |
| Z.AI GLM | `thinking: {type}` | `"disabled"` |
| OpenRouter | `reasoning: {effort}` | `"none"` |
| Ollama | `reasoning_effort` 或 `reasoning: {effort}` | `"none"` |
| Qwen3（vLLM） | `chat_template_kwargs: {enable_thinking}` | `false` |

差异跨越三个维度：字段名不同、值类型不同（字符串 / 布尔 / 嵌套对象）、嵌套层级不同。
Qwen3 的开关甚至位于一个与推理无关的键下，其文档明确写着「not OpenAI API compatible」。

因此不做「按提供商归一化」。归一化要求 client 知道自己在跟谁说话，并要求持续跟踪每家的字段变更；
厂商改一次字段名，映射就会静默失效，而失效表现为「设置没生效」——正是这类配置想消除的失败。
改为一处通用字段：原样并入请求体，覆盖全部现有提供商以及尚未出现的提供商，且不需要任何提供商知识。

- **AC-GEN-003-09**：配置的额外请求体字段必须与内置字段合并后发送，且流式与非流式路径必须发送同一份配置。
- **AC-GEN-003-10**：配置的字段不得覆盖应用自有的字段（`model`、`messages`、`stream`）；写入此类字段必须被拒绝并说明原因。
- **AC-GEN-003-11**：字段值的类型与嵌套结构必须原样保留，不得被强制转换为字符串。

写入时拒绝而非静默丢弃：丢弃会让「已配置」与「已生效」分离，产生无反馈的失败。
运行时另有一道过滤，使绕过写入边界的值（手工改库、直接写设置）退化为「未生效」而不是「请求损坏」。
